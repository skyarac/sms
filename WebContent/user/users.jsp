...
<html >
	...
	<body>
		...
		<table align="center" style="width:980px; background:#26ADFE;" id="headtb">
			...
			<tr style="background:#26ad8f">
				<td>欢迎登陆--[<a href="user/modify.jsp">更改密码</a>|<a href="user/out.jsp">退出系统</a>]</td>
      ...
      // p.setShowPage(1);
			if(null==request.getParameter("showPage")||"0".equals(request.getParameter("showPage"))){
				p.setShowPage(1);
			}else{
       ...
       %>  
			<tr>
				...
				<td><a href="user/modify.jsp?uid=<%=users.getUid()%>">修改</a>|<a href="user/delete.jsp?uid=<%=users.getUid()%>" onclick="return confirm('您确定要删除帐号为[<%=users.getUid() %>]的用户吗?');">删除</a></td>	
			</tr>
			<%
			}
       %>
	...
	<h3 align="center">[<a href="user/pageup.jsp?showPage=<%=p.getShowPage()-1%>">上一页</a>|第<%=p.getShowPage()%>页|<a href="user/pagedown.jsp?showPage=<%=p.getShowPage()+1 %>">下一页</a>|共<%=p.getPageNum() %>页|<a href="user/lastpage.jsp?showPage=<%=p.getPageNum() %>">末页</a>]</h3>
	<h2 align="center"><a href="user/add.jsp">添加用户信息</a></h2>
		<!--登录表格end!-->
	</body>
